...
const schemaDiagnostics = /* #__PURE__ */ defineDiagnostics({
	docsBase,
	reporters: [/* @__PURE__ */ createConsoleReporter()],
	codes: {
		NUXT_B5012: {
			why: (p) => `Invalid \`logLevel\` option: \`${p.value}\`.`,
		...
		NUXT_B5015: {
			why: (p) => `Unknown PostCSS order preset: \`${p.preset}\`.`,
...
var app_default = defineResolvers({
	vue: {
		...
		optionsApi: { async $resolve(val, get) {
	...
	app: {
		...
		buildAssetsDir: { $resolve: (val) => {
			return (typeof val === "string" ? val : process.env.NUXT_APP_BUILD_ASSETS_DIR || "/_nuxt/").replace(/^\/?/, "/").replace(/\/?$/, "/");
		} },
		...
		head: { $resolve: (_val) => {
			var _resolved$meta$find, _resolved$meta$find2;
			...
			if (!((_resolved$meta$find = resolved.meta.find((m) => m === null || m === void 0 ? void 0 : m.charset)) === null || _resolved$meta$find === void 0 ? void 0 : _resolved$meta$find.charset)) resolved.meta.unshift({ charset: resolved.charset || "utf-8" });
			if (!((_resolved$meta$find2 = resolved.meta.find((m) => (m === null || m === void 0 ? void 0 : m.name) === "viewport")) === null || _resolved$meta$find2 === void 0 ? void 0 : _resolved$meta$find2.content)) resolved.meta.unshift({
				name: "viewport",
	...
	unhead: {
		legacy: { $resolve: async (val, get) => {
			if (typeof val !== "boolean") return false;
			if (await get("future.compatibilityVersion") >= 5) {
				if (val) schemaDiagnostics.NUXT_B5013();
				return false;
			}
			return val;
		} },
...
var build_default = defineResolvers({
	...
	logLevel: { $resolve: async (val, get) => {
		if (val && typeof val === "string" && ![
			...
			"info",
			"verbose"
		].includes(val)) schemaDiagnostics.NUXT_B5012({ value: val });
...
//#endregion
//#region src/templates/loading.ts
const _messages = {
	"appName": "Nuxt",
	"loading": "Loading",
	"version": "4.0"
...
var esbuild_default = defineResolvers({ esbuild: { options: {
	target: { $resolve: async (val, get) => {
		...
		if (await get("experimental").then((r) => (r === null || r === void 0 ? void 0 : r.decorators) === true)) return "es2024";
		return "esnext";
	} },
	jsxFactory: "h",
	...
	tsconfigRaw: { $resolve: async (_val, get) => {
		...
		if (!await get("experimental").then((r) => (r === null || r === void 0 ? void 0 : r.decorators) === true)) return val;
...
var experimental_default = defineResolvers({
	future: {
		...
		typescriptBundlerResolution: { async $resolve(val, get) {
			val = typeof val === "boolean" ? val : await get("experimental").then((e) => e === null || e === void 0 ? void 0 : e.typescriptBundlerResolution);
			if (typeof val === "boolean") return val;
			const setting = await get("typescript.tsConfig").then((r) => {
				var _r$compilerOptions;
				return r === null || r === void 0 || (_r$compilerOptions = r.compilerOptions) === null || _r$compilerOptions === void 0 ? void 0 : _r$compilerOptions.moduleResolution;
			});
	...
	features: {
		inlineStyles: { async $resolve(_val, get) {
			const val = typeof _val === "boolean" || typeof _val === "function" ? _val : await get("experimental").then((e) => e === null || e === void 0 ? void 0 : e.inlineSSRStyles);
		...
		noScripts: { async $resolve(val, get) {
			...
			return val === true ? "production" : val === false || isValidLiteral(val) ? val : await get("experimental").then((e) => (e === null || e === void 0 ? void 0 : e.noScripts) && "production") ?? false;
		} }
	},
	experimental: {
		...
		emitRouteChunkError: { $resolve: (val) => {
			...
			if (typeof val === "string" && (/* @__PURE__ */ new Set([
				"manual",
				"automatic",
				"automatic-immediate"
			])).has(val)) return val;
		...
		*/
		...
		typedPages: false,
		appManifest: true,
		checkOutdatedBuildInterval: 36e5,
		...
		spaLoadingTemplateLocation: { $resolve: (val) => {
			return typeof val === "string" && (/* @__PURE__ */ new Set(["body", "within"])).has(val) ? val : "body";
		} },
		...
		ssrStreaming: { $resolve(val) {
			const defaultBotRegex = /bot\b|crawl|spider|slurp|facebookexternalhit|google\b|bing\b|yandex\b|baidu\b|duckduck/i;
			const obj = val !== null && typeof val === "object" ? val : null;
			if (!(val === true || obj !== null && obj.enabled !== false)) return {
				enabled: false,
				botRegex: defaultBotRegex
			};
			return {
				enabled: true,
				botRegex: (obj === null || obj === void 0 ? void 0 : obj.botRegex) instanceof RegExp ? obj.botRegex : defaultBotRegex
			};
		} },
...
var nitro_default = defineResolvers({
	...
	nitro: {
		...
		tracingChannel: { $resolve: async (val, get) => {
			if (val === false) return false;
			...
			if (!base && !override) return val === true ? {} : false;
			return {
				...base || {},
				...override || {}
			};
	...
	serverHandlers: [],
	devServerHandlers: [],
	tracingChannel: { $resolve: (val) => {
		if (val === true) return { nuxt: true };
...
var postcss_default = defineResolvers({ postcss: {
	order: { $resolve: (val) => {
		if (typeof val === "string") {
			if (!(val in orderPresets)) throw schemaDiagnostics.NUXT_B5015({ preset: val });
...
var vite_default = defineResolvers({ vite: {
	...
	resolve: { extensions: [
		...DEFAULT_JS_FILE_EXTENSIONS,
		".vue",
		".json"
	] },
	publicDir: { $resolve: (val) => {
		if (val) schemaDiagnostics.NUXT_B5014();
		return false;
	} },
	vue: {
		...
		script: { hoistStatic: { $resolve: async (val, get) => {
			var _await$get$compilerOp;
			return typeof val === "boolean" ? val : (_await$get$compilerOp = (await get("vue")).compilerOptions) === null || _await$get$compilerOp === void 0 ? void 0 : _await$get$compilerOp.hoistStatic;
		} } },
		features: { propsDestructure: { $resolve: async (val, get) => {
			var _vueOptions$script;
			if (typeof val === "boolean") return val;
			const vueOptions = await get("vue") || {};
			return Boolean(((_vueOptions$script = vueOptions.script) === null || _vueOptions$script === void 0 ? void 0 : _vueOptions$script.propsDestructure) ?? vueOptions.propsDestructure);
		} } }
	},
	vueJsx: { $resolve: async (val, get) => {
		var _await$get$compilerOp2;
		const options = val && typeof val === "object" ? val : {};
		return {
			isCustomElement: (_await$get$compilerOp2 = (await get("vue")).compilerOptions) === null || _await$get$compilerOp2 === void 0 ? void 0 : _await$get$compilerOp2.isCustomElement,
			defineComponentName: [.../* @__PURE__ */ new Set([...options.defineComponentName ?? ["defineComponent"], "defineNuxtComponent"])],
			...options
		};
	} },
	optimizeDeps: { exclude: { $resolve: (val) => [...Array.isArray(val) ? val : [], "vue-demi"] } },
	clearScreen: true,
	build: {
		assetsDir: { $resolve: async (val, get) => {
			var _await$get$buildAsset;
			return typeof val === "string" ? val : (_await$get$buildAsset = (await get("app")).buildAssetsDir) === null || _await$get$buildAsset === void 0 ? void 0 : _await$get$buildAsset.replace(/^\/+/, "");
		} },
		emptyOutDir: false
	},
	...
	cacheDir: { $resolve: async (val, get) => {
		if (typeof val === "string") return val;
		const rootDir = await get("rootDir");
		if (existsSync(resolve(rootDir, "node_modules"))) return resolve(rootDir, "node_modules/.cache/vite");
		const workspaceDir = await get("workspaceDir");
		const relativeRoot = relative(workspaceDir, rootDir);
		if (!relativeRoot || relativeRoot.startsWith("..")) return resolve(rootDir, "node_modules/.cache/vite");
