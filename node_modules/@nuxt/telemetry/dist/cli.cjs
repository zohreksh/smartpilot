...
const env = require('./shared/telemetry.2lRE61Nh.cjs');
  ...
  subCommands: {
    ...
    enable: citty.defineCommand({
      ...
      async run({ args }) {
        ...
        setRC(dir, "telemetry.consent", env.consentVersion, args.global);
        await showStatus(dir, args.global);
    ...
    consent: citty.defineCommand({
      ...
      async run({ args }) {
        await ensureNuxtProject(args);
        const dir = node_path.resolve(args.dir);
        const accepted = await env.ensureUserconsent({});
...
async function _checkDisabled(dir) {
  ...
  if (env.isTruthy(process.env.NUXT_TELEMETRY_DISABLED)) {
  ...
  try {
    ...
    for (const layer of config._layers) {
      if (disabledByConf(layer.config)) {
        return "by " + (layer.configFile ?? config._layers[0].configFile);
      }
    }
  } catch {
  }
  if (disabledByConf(rc__namespace.read({ name: env.RC_FILENAME, dir }))) {
    return "by " + node_path.resolve(dir, env.RC_FILENAME);
  }
  if (disabledByConf(rc__namespace.readUser({ name: env.RC_FILENAME }))) {
    return "by " + node_path.resolve(os.homedir(), env.RC_FILENAME);
  }
}
...
function setRC(dir, key, val, global) {
  ...
  if (global) {
    rc__namespace.updateUser(update, env.RC_FILENAME);
  } else {
    rc__namespace.update(update, { name: env.RC_FILENAME, dir });
  }
}
