...
async function postEvent(endpoint, body) {
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "user-agent": "Nuxt Telemetry " + env.version
    },
...
function hash(str) {
  return node_crypto.createHash("sha256").update(str).digest("hex").slice(0, 16);
}
function randomSeed() {
...
async function createContext(nuxt, nitro, options) {
  ...
  return {
    ...
    cli: getCLI(),
    nodeVersion,
    os: os__default.type().toLowerCase(),
    environment: getEnv(),
    packageManager,
...
function getGitRemote(cwd) {
  try {
    return node_child_process.execSync("git config --get remote.origin.url", { encoding: "utf8", cwd, stdio: ["ignore", "pipe", "ignore"] }).trim() || null;
  } catch {
    return null;
  }
}
...
const module$1 = kit.defineNuxtModule({
  ...
  async setup(toptions, nuxt) {
    ...
    if (env.isTruthy(process.env.NUXT_TELEMETRY_DISABLED)) {
      logger.info("Telemetry disabled by `NUXT_TELEMETRY_DISABLED` environment variable");
      return;
    }
    const _topLevelTelemetry = nuxt.options.telemetry;
    if (_topLevelTelemetry !== true) {
      if (toptions.enabled === false || _topLevelTelemetry === false || !await env.ensureUserconsent(toptions)) {
    ...
    if (!toptions.seed || typeof toptions.seed !== "string") {
      ...
      env.updateUserNuxtRc("telemetry.seed", toptions.seed);
