import { IS_CSS_RE, isCSS, isVue, parseModuleId, toArray, toVirtualId, writeManifest } from "./_chunks/manifest.mjs";
...
function pickSocketPath(platform, tmpdir = os.tmpdir()) {
	...
	if (platform === "win32") return { socketPath: win32.join(String.raw`\\.\pipe`, socketDir + randomUUID().slice(0, 8)) };
...
function createViteNodeSocketServer(nuxt, ssrServer, clientServer, invalidates, config) {
	const server = net.createServer((socket) => {
		...
		const MAX_BUFFER_SIZE = 1073741824;
...
function wrapStringGenerateScopedName(pattern, hashPrefix) {
...
function SSRStylesPlugin(nuxt) {
	...
	const withInlineQuery = (id) => {
		const match = id.match(/([?&])lang\.[^&?]+$/);
		if (match) return id.slice(0, match.index) + match[1] + "inline&used&" + id.slice(match.index + 1);
	...
	const chunkNamePrefixes = /* @__PURE__ */ new Map();
	const usedChunkNamePrefixes = /* @__PURE__ */ new Set();
	const options = {
	...
	const flattenPages = (pages) => pages?.flatMap((p) => [p, ...flattenPages(p.children)]) ?? [];
	const serverPages = flattenPages(nuxt.apps.default.pages).filter(({ mode, file }) => mode === "server" && file);
			...
			return {
				...
				generateBundle(outputOptions) {
					...
					for (const [file, { files, inBundle }] of Object.entries(cssMap)) {
						...
						const baseName = filename$1(file);
						let assetName = `${baseName}-styles.mjs`;
						for (let i = 2; usedNames.has(assetName); i++) assetName = `${baseName}-styles-${i}.mjs`;
				...
				renderChunk(_code, chunk) {
					...
					for (const moduleId of [chunk.facadeModuleId, ...chunk.moduleIds].filter(Boolean)) {
						...
						if (relativePath in cssMap) cssMap[relativePath].inBundle = cssMap[relativePath].inBundle ?? (isVue(moduleId.replace(QUERY_RE, "")) && !!relativePath || isEntry);
					}
					return null;
				},
				transform: {
					...
					async handler(code, id) {
						...
						if (chunkNamePrefix === void 0) {
							const baseName = filename$1(id) || "styles";
							chunkNamePrefix = baseName;
							for (let i = 2; usedChunkNamePrefixes.has(chunkNamePrefix); i++) chunkNamePrefix = `${baseName}-${i}`;
