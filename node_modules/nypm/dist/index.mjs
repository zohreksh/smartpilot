...
function getWorkspaceArgs(options) {
	...
	if (options.packageManager.name === "pnpm" || options.packageManager.name === "aube" || options.packageManager.name === "nub") return workspacePkg ? ["--filter", workspacePkg] : ["--workspace-root"];
	if (options.packageManager.name === "npm") return workspacePkg ? ["-w", workspacePkg] : ["--workspaces"];
	if (options.packageManager.name === "yarn") {
		if (!options.packageManager.majorVersion || options.packageManager.majorVersion === "1") return workspacePkg ? ["--cwd", workspacePkg] : ["-W"];
...
function getWorkspaceArgs2(options) {
	if (!options.workspace) return [];
	const workspacePkg = typeof options.workspace === "string" && options.workspace !== "" ? options.workspace : void 0;
	if (options.packageManager === "pnpm" || options.packageManager === "aube" || options.packageManager === "nub") return workspacePkg ? ["--filter", workspacePkg] : ["--workspace-root"];
	if (options.packageManager === "npm") return workspacePkg ? ["-w", workspacePkg] : ["--workspaces"];
	if (options.packageManager === "yarn") {
		if (options.yarnBerry) return workspacePkg ? ["workspace", workspacePkg] : [];
...
function parsePackageManagerField(packageManager) {
	...
	if (name && name !== "-" && VALID_PM_NAME_RE.test(name)) return {
		name,
		version,
		buildMeta
	};
...
function parseDevEnginesPackageManager(devEngines) {
	...
	const name = typeof entry?.name === "string" ? entry.name : void 0;
	...
	if (!VALID_PM_NAME_RE.test(name)) {
		...
		return {
			name: sanitized,
			version,
			warnings: [`Abnormal characters found in \`devEngines.packageManager\` field, sanitizing from \`${name}\` to \`${sanitized}\``]
		};
	}
	return {
		name,
