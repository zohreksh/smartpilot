...
function getContentType(asType, extension) {
	if (asType === "font") return `font/${extension}`;
	if (asType === "image") return contentTypeMap[extension] || `image/${extension}`;
}
function getAsType(ext) {
	if (ext === "js" || ext === "cjs" || ext === "mjs") return "script";
	else if (ext === "css") return "style";
	else if (IMAGE_RE.test(ext)) return "image";
	else if (FONT_RE.test(ext)) return "font";
	else if (AUDIO_RE.test(ext)) return "audio";
	else if (VIDEO_RE.test(ext)) return "video";
}
const parseResource = (path) => {
	...
	const extension = dotIndex === -1 || dotIndex < path.lastIndexOf("/", end - 1) ? "" : path.slice(dotIndex + 1, end);
	const asType = getAsType(extension);
	if (asType) {
		chunk.resourceType = asType;
		if (asType === "script" && extension !== "cjs") chunk.module = true;
	}
	if (chunk.resourceType !== "font") chunk.prefetch = true;
	if (chunk.resourceType === "script" || chunk.resourceType === "style") chunk.preload = true;
	const contentType = getContentType(asType, extension);
	if (contentType) chunk.mimeType = contentType;
	return chunk;
};
//#endregion
//#region src/vite.ts
function normalizeViteManifest(manifest) {
	...
	for (const file in manifest) {
		...
		for (const item of chunk.css || []) if (!_manifest[item]) _manifest[item] = {
			file: item,
			...parseResource(item)
		};
		for (const item of chunk.assets || []) if (!_manifest[item]) _manifest[item] = {
			file: item,
			...parseResource(item)
		};
	}
...
function normalizeWebpackManifest(manifest) {
	...
	for (const outfile of manifest.all) if (isJS(outfile)) clientManifest[getIdentifier(outfile)] = {
		file: outfile,
		...parseResource(outfile)
	};
	const first = getIdentifier(manifest.initial.find(isJS));
	if (first) {
		if (!(first in clientManifest)) throw new Error(`Invalid manifest - initial entrypoint not in \`all\`: ${manifest.initial.find(isJS)}`);
		clientManifest[first].css = [];
		clientManifest[first].assets = [];
		clientManifest[first].dynamicImports = [];
	}
	for (const outfile of manifest.initial) if (isJS(outfile)) clientManifest[getIdentifier(outfile)].isEntry = true;
	...
	for (const outfile of manifest.async) if (isJS(outfile)) {
		const identifier = getIdentifier(outfile);
		if (!(identifier in clientManifest)) throw new Error(`Invalid manifest - async module not in \`all\`: ${outfile}`);
	...
	for (const [moduleId, importIndexes] of Object.entries(manifest.modules)) {
		...
		for (const index of importIndexes) {
			...
			} else if (isCSS(file)) cssFiles.push(file);
		...
		for (const key of ["css", "assets"]) for (const file of clientManifest[moduleId][key] || []) clientManifest[file] = clientManifest[file] || {
			file,
			...parseResource(file)
		};
	}
...
function precomputeDependencies(manifest) {
	...
	function computeDependencies(id) {
		...
		if (meta.file) {
			if (meta.preload) deps.preload[id] = meta;
			if (meta.isEntry || meta.sideEffects) deps.scripts[id] = meta;
		}
		for (const css of meta.css || EMPTY) {
			const cssResource = manifest[css];
			if (cssResource) {
				deps.styles[css] = cssResource;
				if (cssResource.preload) deps.preload[css] = cssResource;
		...
		for (const asset of meta.assets || EMPTY) {
			...
			if (assetResource) {
				if (assetResource.preload) deps.preload[asset] = assetResource;
	...
	for (const moduleId in manifest) {
		...
		if (meta.isEntry) entrypoints.push(moduleId);
		const module = {
			file: meta.file,
		...
		if (meta.dynamicImports?.length) module.dynamicImports = meta.dynamicImports;
