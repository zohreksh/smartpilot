...
export async function loadPayload(url, opts = {}) {
  ...
  if (await shouldLoadPayload(url)) {
    ...
    const cache = isCachedPayloadRoute(url) ? "default" : "force-cache";
...
function _getRoutePath(url) {
  return new URL(url, "http://localhost").pathname;
}
export function isCachedPayloadRoute(url) {
...
export async function getNuxtClientPayload() {
  ...
  const externalData = el.dataset.src ? await _importPayload(el.dataset.src, inlineData.prerenderedAt ? "force-cache" : "no-cache") : void 0;
  payloadCache = {
    ...inlineData,
    ...externalData,
