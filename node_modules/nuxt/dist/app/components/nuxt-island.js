...
import { computeIslandHash, serializeIslandProps } from "../island-hash.js";
  ...
  async setup(props, { slots, expose, emit }) {
    ...
    function setPayload(key2, result) {
      ...
      nuxtApp.payload.data[key2] = {
        __nuxt_island: {
          key: key2,
          ...import.meta.server && import.meta.prerender ? {} : { params: { ...props.context, props: props.props ? serializedProps.value : void 0 } },
          result: toRevive
        },
        ...result
      };
      ...
      try {
        ...
        if (import.meta.server && res.components && Object.keys(res.components).length) {
          ...
          if (parentIslandContext) {
            for (const [id2, { html: _, ...component }] of Object.entries(res.components)) {
              parentIslandContext.components[id2] = { ...component, uid: uid.value };
            }
          }
        }
              ...
              if (import.meta.server) {
                if (payloads.components) {
                  for (const [id2, info] of Object.entries(payloads.components)) {
                    const { html: html2, slots: slots2, uid: targetUID = uid.value } = info;
                    ...
                    teleports.push(createVNode(Teleport, { to: `uid=${targetUID};client=${id2}` }, {
                ...
                for (const [id2, info] of Object.entries(payloads.components)) {
                  const { props: props2, slots: slots2, uid: targetUID = uid.value } = info;
                  const component = components.get(id2);
                  const vnode = createVNode(Teleport, { to: `${isKeyOdd ? "div" : ""}[data-island-uid='${targetUID}'][data-island-component="${id2}"]` }, {
                    default: () => {
