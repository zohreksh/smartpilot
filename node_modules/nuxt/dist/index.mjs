...
import { useLogger, tryUseNuxt, useNuxt, directoryToURL, getLayerDirectories, resolveFiles, resolvePath, resolveAlias, defineNuxtModule, findPath, addPlugin, addTypeTemplate, addTemplate, addComponent, useNitro, addBuildPlugin, isIgnored, addPluginTemplate, addImportsSources, addVitePlugin, createIsIgnored, updateTemplates, tryResolveModule, normalizeModuleTranspilePath, importModule, createResolver, runWithNuxtContext, nuxtCtx, loadNuxtConfig, installModules, resolveIgnorePatterns, addRouteMiddleware, normalizeTemplate, compileTemplate, normalizePlugin, templateUtils } from '@nuxt/kit';
...
import { genArrayFromRaw, genSafeVariableName, genImport, genDynamicImport, genObjectKey, genInlineTypeImport, genObjectFromRawEntries, genString, genDynamicTypeImport, genExport } from 'knitwork';
import { resolveModulePath } from 'exsolve';
import { addDependency } from 'nypm';
import { reverseResolveAlias, filename, resolveAlias as resolveAlias$1 } from 'pathe/utils';
...
import { parseAndWalk, ScopeTracker, walk, isReferenceIdentifier, getUndeclaredIdentifiersInFunction } from 'oxc-walker';
            ...
            for (const decl of scopeTrackerNode.variableNode.declarations) {
              ...
              walk(decl.init, {
                enter: (node, parent) => {
                  ...
                  if (!isReferenceIdentifier(node, parent) || node.type !== "Identifier") {
                    return;
                  }
        ...
        walk(ast, {
          ...
          enter: (node) => {
            ...
            walk(meta, {
              scopeTracker,
              enter(node2, parent) {
                if (!isReferenceIdentifier(node2, parent) || node2.type !== "Identifier") {
                  return;
                }
...
function normalizeRouteRulePath(path, fold) {
    ...
    nuxt.hook("nitro:build:before", (nitro) => {
      ...
      if (!nitro.options.static) {
        ...
        for (const [route, rules] of Object.entries(nitro.options.routeRules)) {
          addRoute(routeRulesRouter, void 0, normalizeRouteRulePath(route, !caseSensitiveRouteRules), rules);
        }
        for (const route of prerenderRoutes) {
          const rules = defu({}, ...findAllRoutes(routeRulesRouter, void 0, normalizeRouteRulePath(route, !caseSensitiveRouteRules)).reverse());
          if (rules.prerender) {
...
const metaModule = defineNuxtModule({
  ...
  setup(options, nuxt) {
    ...
    const resolveNuxtUnhead = (id) => resolveModulePath(id, { from: import.meta.url });
...
function boundVForExpression(expression) {
      ...
      async handler(code, id) {
        ...
        await walk$1(ast, (node) => {
          ...
          if (node.name === "slot") {
            ...
            attributes2["v-for"] &&= boundVForExpression(attributes2["v-for"]);
            ...
            if (children.length) {
              ...
              s.overwrite(startingIndex + loc2[0].start, startingIndex + loc2[0].end, boundVForInTag(code.slice(startingIndex + loc2[0].start, startingIndex + loc2[0].end).replaceAll(EXTRACTED_ATTRS_RE, "")));
            }
    ...
    transform: {
      ...
      handler(code, id) {
        ...
        parseAndWalk(code, id, (node, parent) => {
          ...
          const filePath = resolveAlias$1(rawPath, options.alias || {});
    ...
    nuxt.hook("app:resolve", async () => {
      ...
      for (const dir of allDirs) {
        ...
        if (nuxt.options.dev && dirOptions.watch !== false && !inNodeModules) {
          const coveredByLayer = getLayerDirectories(nuxt).some(
            (dirs2) => dirPath === dirs2.app.replace(/\/$/, "") || dirPath.startsWith(dirs2.app)
          );
    ...
    await Promise.all(importSource.map(async (from) => {
      ...
      if (!isAbsolute(path)) {
        path = await tryResolveModule(from, importPaths).then(async (r2) => {
          ...
          return subpath && subpath !== "./" ? join(dir, name, subpath) : r2;
        }) ?? path;
      }
          ...
          enter(node) {
            processKeyedFunction(this, node, ({ parsedCall, fnMeta }) => {
              ...
              switch (parsedCall.name) {
                case "useState":
                  if (stringTypes.includes(parsedCall.callExpression.arguments[0]?.type) && typeof fnMeta.source === "string" && stripExtension(fnMeta.source) === stripExtension(resolveAlias("#app/composables/state", options.alias))) {
                    return;
                  }
                  break;
                case "useFetch":
                case "useLazyFetch":
                  if (stringTypes.includes(parsedCall.callExpression.arguments[1]?.type) && typeof fnMeta.source === "string" && stripExtension(fnMeta.source) === stripExtension(resolveAlias("#app/composables/fetch", options.alias))) {
                    return;
                  }
                  break;
                case "useAsyncData":
                case "useLazyAsyncData":
                  if (stringTypes.includes(parsedCall.callExpression.arguments[0]?.type) && typeof fnMeta.source === "string" && stripExtension(fnMeta.source) === stripExtension(resolveAlias("#app/composables/asyncData", options.alias))) {
                    return;
                  }
                  break;
              }
...
Using \`${fallbackCompatibilityDate}\` as fallback. More info at: ${colors.underline("https://nitro.build/deploy#compatibility-date")}`);
  ...
  const pagesEnabled = nuxt.options.pages !== false && nuxt.options.pages?.enabled !== false;
