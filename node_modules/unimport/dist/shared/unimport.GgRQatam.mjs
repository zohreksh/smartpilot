import { V as VUE_TEMPLATE_NAME, a as vueTemplateAddon, y as VUE_DIRECTIVES_NAME, v as vueDirectivesAddon, i as getMagicString, o as stripCommentsAndStrings, R as RE_EXCLUDE, d as RE_SEPARATOR, b as RE_IMPORT_AS, c as RE_MATCH, g as defineUnimportPreset, t as toExports, n as normalizeImports, f as dedupeImports, u as toTypeDeclarationFile, x as toTypeReExports, e as addImportToCode, p as stripFileExtension } from './unimport.B4o9Mg_h.mjs';
...
async function detectImportsRegex(code, ctx, options) {
  ...
  if (options?.autoImport !== false) {
    ...
    for (const i of strippedCode.matchAll(RE_MATCH)) {
      ...
      if (prev === void 0 || occurrence < prev)
...
function getForLoopDeclarationRange(code, match) {
...
function findMatchingCharacter(code, start, open, close) {
  let depth = 0;
...
async function detectImports(code, ctx, options, id) {
  switch (options?.parser) {
    case "acorn":
      return import('../chunks/detect-acorn.mjs').then((r) => r.detectImportsAcorn(code, ctx, options, id));
    case "oxc":
      return import('../chunks/detect-oxc.mjs').then((r) => r.detectImportsOxc(code, ctx, options, id));
    default:
...
async function scanFilesFromDir(dir, options) {
  ...
  const dirMatchers = dirGlobs.map((glob2) => pm(glob2));
...
async function canSkipImportDetection(code, ctx, options) {
  if (options?.parser && options.parser !== "regex")
    return false;
  if (ctx.addons.some(
    (addon) => addon.matchImports || addon.injectImportsResolved || addon.injectImportsStringified
  )) {
    return false;
  }
  if (options?.transformVirtualImports !== false && ctx.options.virtualImports?.some((name) => code.includes(name))) {
    return false;
  }
  if (options?.autoImport === false)
    return true;
...
export { resolvePreset as a, builtinPresets as b, createUnimport as c, dedupeDtsExports as d, scanExports as e, scanFilesFromDir as f, normalizeScanDirs as n, resolveBuiltinPresets as r, scanDirExports as s, version as v };
