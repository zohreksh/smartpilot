...
import { basename as u, delimiter as d, dirname as f, normalize as p, resolve as m } from "node:path";
import { cwd as h } from "node:process";
...
function M(e, t) {
	let n = t.value.split(d), r = [], i = e, a;
	do
		r.push(m(i, `node_modules`, `.bin`)), a = i, i = f(i);
	while (i !== a);
	r.push(f(process.execPath));
	let o = r.concat(n).join(d);
	return {
		key: t.key,
		value: o
	};
...
function V(e, n = [], a = {}) {
	...
	if (o !== null) {
		...
		if (a !== null) {
			let e = a[1].trim(), t = e.indexOf(` `), n = t === -1 ? e : e.slice(0, t), r = t === -1 ? `` : e.slice(t + 1), i = u(n);
			s = i === `env` ? r || null : i;
		}
	}
	if (s !== null && o !== null && (n = [o, ...n], e = s, o = H(e, a)), o === null || !L.test(o)) {
		let t = o !== null && R.test(o);
		e = p(e), e = e.replace(F, `^$1`), n = n.map((e) => (e = e.replace(/(?=(\\+?)?)\1"/g, `$1$1\\"`), e = e.replace(/(?=(\\+?)?)\1$/, `$1$1`), e = `"${e}"`, e = e.replace(F, `^$1`), t && (e = e.replace(F, `^$1`)), e)), n = [
			`/d`,
			`/s`,
			`/c`,
			`"${[e, ...n].join(` `)}"`
...
function H(e, t) {
	let n = (t.cwd ?? h()).toString(), r = t.env ?? process.env, i = j(r).value, o = e.includes(`/`) || e.includes(`\\`) ? [``] : [n, ...i.split(d)], s = r.PATHEXT ? r.PATHEXT.split(d) : B;
	e.includes(`.`) && s[0] !== `` && s.unshift(``);
	for (let t of o) {
		let r = m(n, t.startsWith(`"`) && t.endsWith(`"`) && t.length > 1 ? t.slice(1, -1) : t, e);
		for (let e of s) {
			let t = r + e;
			try {
...
var q = class {
	...
	async *[Symbol.asyncIterator]() {
		...
		let n = P(t), r = g.createInterface({ input: n });
	...
	spawn() {
		...
		let i = V(this._command, this._args, n), a = l(i.command, i.args, i.options);
...
async function $(e, t = {}) {
	let r = await ie(E(e || `.`), async (e) => {
		if (!t.ignorePackageJSON) {
			...
			if (n(w(e, `deno.json`))) return Q.find((e) => e.name === `deno`);
		}
		if (!t.ignoreLockFile) {
			for (let t of Q) if ([t.lockFile, t.files].flat().filter(Boolean).some((t) => n(E(e, t)))) return { ...t };
		}
...
export { ne as basename, te as dirname, X as dist_exports, ee as relative, E as resolve };
